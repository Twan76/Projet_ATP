...
ranking_par_date <- function(x) {
  ...
  for(i in 1:length(x)) {
    ...
    if(nrow(ranking_atp)>0) { # Pas de classement semaine de grand chelem
      ranking_atp <- ranking_atp[,c(1,2,4,5,6,9)]
      ranking_atp$Date <- x[i]
      colnames(ranking_atp) <- c("Rang","Diff rang / sem. der.", "Joueur", "Age", "Points", "Points à défendre", "Date")
  ...
  ranking_atp_maj[,5] <- str_replace_all(ranking_atp_maj[,5],",","")
...
write_rds(ranking_atp_annee, "ranking_atp_annee.rds")
